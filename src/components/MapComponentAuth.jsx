...
export default function MapComponent(){
    ...
    const assignColor = (
        ...
        const result = data.map((elem) => {
            if (elem._count.geodatas != 0) {
                if (elem._count.geodatas > Math.round(calc.upper.val)) {
                    calc.upper.exist = true;
                    elem.color = "green";
                    return elem;
                }
                if (elem._count.geodatas < Math.round(calc.lower.val)) {
                    calc.lower.exist = true;
                    elem.color = "red";
                    return elem;
                }
                if (
                    elem._count.geodatas >= Math.round(calc.lower.val) &&
                    elem._count.geodatas <= Math.round(calc.upper.val)
                ) {
                    elem.color = "yellow";
                    return elem;
                }
